...
function getCacheKey(location, placeType, radius) {
...
function formatPlaces(places, locationName, isMock, radiusKm, keyword) {
    let header = isMock
...
function generateMockPlaces(placeType, radius) {
    const names = {
...
function convertTo24Hour(timeStr) {
    ...
    if (!match)
        return timeStr; // if no match, return original
...
function evaluateWeatherCondition(conditionStr, currentWeather, currentTempF) {
    if (!currentWeather && currentTempF === null)
        return false;
...
server.resource("hello-world", "hello://world", async (uri) => ({
    contents: [{
            uri: uri.href,
...
server.tool("calendar", "Manage calendar events - create, list, check availability, and delete events", {
    ...
    switch (action) {
        ...
        case "list_events":
            ...
            const eventList = events.map((event) => `• ${event.title} - ${event.date} at ${event.time} (${event.duration}min)`).join('\n');
            return {
                content: [{
                        type: "text",
        ...
        case "check_availability":
            ...
            const requestedDateTime = new Date(parseDate(date).setHours(parseInt(time24h.split(':')[0]), parseInt(time24h.split(':')[1])));
            ...
            const hasConflict = events.some((event) => {
                ...
                const eventStart = new Date(eventDate.setHours(parseInt(eventTime24h.split(':')[0]), parseInt(eventTime24h.split(':')[1])));
...
server.tool("weather", "Get current weather forecasts for any city worldwide", {
    city: z.string({
...
• **Visibility**: 18 km`
        ...
        const mockData = mockWeather[normalizedCity];
        if (mockData) {
            return {
                content: [{
                        type: "text",
...
server.tool("get_current_location", "Get your approximate current location based on IP address", {}, async () => {
    try {
...
server.tool("find_nearby_places", "Find restaurants, cafes, hotels, and other places near a location using Google Places", {
    ...
    try {
        ...
        let places = placesData.results.map((place) => {
        ...
        if (min_rating !== undefined && min_rating > 0) {
            places = places.filter((p) => p.ratingValue >= min_rating);
        }
...
server.tool("create_smart_reminder", "Create a reminder that can trigger based on time, location, weather, or any combination", {
    message: z.string(),
...
server.tool("check_reminders", "Check which smart reminders are due based on current time, location, and weather", {}, async () => {
    const reminders = await loadReminders();
    const dueReminders = [];
    // ---- Get current context ----
    const now = new Date();
